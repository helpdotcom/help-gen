...
module.exports = function biscuits(obj, cb) {
  ...
  const ___2 = typeof obj['room']
  if (!obj['room'] || typeof obj['room'] !== 'object') {
    return setImmediate(() => {
      cb(new TypeError(`Missing or invalid required param (room) for name room. Expected object, got ${ ___2 }`))
    })
  }
  const ___3 = typeof obj['room']['id']
  if (typeof obj['room']['id'] !== 'string') {
    return setImmediate(() => {
      cb(new TypeError(`Missing or invalid required param (room.id) for name roomId. Expected string, got ${ ___3 }`))
    })
  }
