...
function processMap(map) {
  ...
  for (const item of map) {
    ...
    if (is.isMap(val)) {
      ...
      const objectNode = utils.objectExpression(nodes)
...
function getValue(item) {
  if (Array.isArray(item)) {
    return Builder.array(item.map((thing) => {
      return getValue(thing)
    }))
  } else if (item && typeof item === 'object') {
    // This is for examples. This should only be reached if an example
    // is an array of objects.
    return utils.objectExpression(Object.keys(item).map((key) => {
      if (typeof item[key] === 'object') {
        return getValue(item[key])
      }
      return utils.property(Builder.id(key), getValue(item[key]), 'init')
    }))
  } else {
    const t = typeof item
    switch (t) {
