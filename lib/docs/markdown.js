...
Markdown.prototype._writeCurl = function _writeCurl() {
  if (!this.json.curl) {
    return
  }
  this._line('#### Request')
  this._line()
  this._line(this._toCode(this.json.curl))
}
...
Markdown.prototype._writeParam = function _writeParam(param) {
  const name = this._toCodeString(param.path)
  if (param.type === 'enum') {
    param.values = param.values || param.value
    delete param.value
  }
  const type = param.type === 'enum'
    ? this._toCodeString(`enum [${param.values.map((item) => {
        return JSON.stringify(item)
      }).join(', ')}]`)
    : this._toCodeString(param.type === 'uuid'
    ? 'uuid (v4)'
    : param.type)
