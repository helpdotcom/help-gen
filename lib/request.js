...
Request.prototype.nextVar = function nextVar() {
  return `___${this._currentVar++}`
}

Request.prototype._addRegExp = function _addRegExp(re) {
...
Request.prototype._addEnum = function _addEnum(prop) {
...
Request.prototype._processModule = function _processModule() {
...
Request.prototype._processBody = function _processBody() {
  ...
  this.props.forEach((item) => {
    ...
    // TODO(evanlucas) we should be checking configs that are not required too.
    // If they are not required, we should check that they pass validation
    // if they exist.
    if (item.required)
...
module.exports = function createValidator(name, props) {
  return new Request(name, props).generate()
}

module.exports.Request = Request

Request.prototype.typeofCheckForPath = function typeofCheckForPath(opts, a) {
...
Request.prototype.addSingleTypeofCheck = function addSingleTypeofCheck(o, a) {
  const opts = o
  ...
  if (checks.isBuiltinType(opts.type)) {
    ...
    builder
      ...
      .body.push(ast.valTypeofCheck(opts.type, opts.path, v))
    ...
    if (opts.type === 'regex') {
      ...
      const em = `Path "${opts.path}" must be one of [${poss}]`
      const err = Builder.new('Error', [
        Builder.string(em)
      ])
      ...
      builder.body.push(ast.valTypeofCheck(opts.type, opts.path))
    }
  } else {
