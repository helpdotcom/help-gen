...
exports.syncTypeofCheck = function syncTypeofCheck(name, type, path, varName) {
  ...
  if (type === 'object') {
    ...
    const left = exports.notExpression(path, true)
    const right = {
    ...
    if (varName) {
      ...
      const block = exports.getInvalidBlocks(exports.identifier(err))
      return exports.ifOr(left, right, block)
    }

  } else if (type === 'uuid' || type === 'date') {
    let args = [
      exports.memberExpression(path, true)
    ]

    const fnName = type === 'uuid'
      ? 'isv4UUID'
      : 'isDate'

    let block = exports.getInvalidBlocks(exports.literal(err))
    return {
      type: 'IfStatement'
    , test: {
        type: 'UnaryExpression'
      , operator: '!'
      , prefix: true
      , argument: exports.callExpression(fnName, args)
      }
    , consequent: block
    }
  } else {
...
// name is should be the config.name, varName represents the name
// of the variable to which the typeof check is assigned
exports.typeofCheck = function typeofCheck(name, type, path, varName) {
