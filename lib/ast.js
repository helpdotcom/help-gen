...
exports.resCustomTypeofCheck = function resCustomTypeofCheck(path, er, type) {
  ...
  return exports.customTypeofCheck(path, type, block)
}
...
exports.valCustomTypeofCheck = function valCustomTypeofCheck(path, er, type) {
  ...
  return exports.customTypeofCheck(`obj.${path}`, type, block)
}

exports.customTypeofCheck = function customTypeofCheck(path, type, block) {
  if (type === 'array') {
    const arg = exports.callExpression(getFunctionName(type), [
      ast.objectPath(path)
    ])
    return exports.ifNot(arg, block)
  }

  const fn = is[getFunctionName(type)]
  return exports.ifNot(fn(path), block)
}
...
// used for responses
exports.resTypeofCheck = function resTypeofCheck(type, path, varName) {
...
// name is should be the config.name, varName represents the name
// of the variable to which the typeof check is assigned
exports.valTypeofCheck = function valTypeofCheck(type, path, varName) {
  let err = `Missing or invalid required param: "${path}".`
  if (varName) {
