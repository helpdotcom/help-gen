...
Response.prototype._modifyAST = function _modifyAST() {
  ...
  ast.body[0].expression.right.name = name

  // Change constructor name to _name_
  ast.body[1].id.name = name

  // Change static MemberExpression from Base to _name_
  ast.body[2].expression.left.object.name = name

  // Change NewExpression constructor to _name_
  ast.body[2].expression.right.body.body[0].argument.callee.name = name

  // Change ExpressionStatement for prototype to _name_
  ast.body[3].expression.left.object.object.name = name
}
...
Response.prototype.generate = function generate() {
  ...
  // shift the top three lines off.
  // we will always add useStrict back, tld and emailRE
  // will only be added back in the event isEmail needs to be called
  ...
  const bodyNode = ast.body[1].body.body

  // This is where we will be adding our validations.
  // It is inside the Base#isValid() function
  const validNode = ast.body[3].expression.right.body.body
  ...
  for (const decl of this.decls) {
    ...
    const right = utils.logicalOr(
      utils.objectPath(`opts.${decl[0]}`, true)
    , obj
    )
...
Response.prototype.addSingleTypeofCheck = function addSingleTypeofCheck(opts) {
  ...
  const ast = this.ast
  const body = ast.body[3].expression.right.body.body
  ...
  if (checks.isBuiltinType(opts.type)) {
    ...
    let mem = utils.typeofExpression(
      utils.objectPath(`this.${opts.path}`, true)
    )
...
function getDeclaration(prop) {
  ...
  const right = utils.objectPath(`opts.${prop.path}`, true)
