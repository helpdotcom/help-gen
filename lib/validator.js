...
module.exports = class Validator {
  ...
  _topLevelArrayCheck() {
    ...
    , Builder.block(this._callbackWithError(Builder.TypeError(em)))
    )
  }
  ...
  _addCustomCheck(prop) {
    ...
    const block = this._callbackWithError(
      this._customTypeError(prop)
    )

    if (prop.allowNull) {
      test = Builder.and(this._checkNull(prop), test)
    }
  ...
  _addArrayCheck(prop) {
    ...
    if (prop.props instanceof NanoProp || typeof prop.props === 'object') {
  ...
  _addCheckForNanoProp(parent) {
    ...
    , Builder.block(this._callbackWithError(
  ...
  _addRegexCheck(prop) {
    ...
    return [Builder.if(
      test
    , Builder.block(this._callbackWithError(err))
    )]
  }
  ...
  _addEnumCheck(prop) {
    ...
    const ppath = prop.path.replace(/\.\^__IDX__1/g, '[i]')
    const em = `(${this.type}): Path "${ppath}" must be one of [${poss}]`
    ...
    return [Builder.if(test, Builder.block(
      this._callbackWithError(err)
    ))]
  }

  _checkNull(prop) {
    const path = this._wrapVar(prop.path)
    return Builder.notEquals(Builder.objectPath(path), Builder.id('null'))
  }
  ...
  _builtinCheck(prop) {
    ...
    const block = this._callbackWithError(this._getBuiltinTypeError(prop))
  ...
  _addRequiredStringCheck(prop) {
    ...
    if (typeof prop.min === 'number' && prop.min) {
      const block = this._callbackWithError(this._getStringMinError(prop))
      builder.if(this._getStringLessThan(prop), Builder.block(block))
    }

    if (typeof prop.max === 'number') {
      const block = this._callbackWithError(this._getStringMaxError(prop))
      builder.if(this._getStringGreaterThan(prop), Builder.block(block))
    }

    if (prop.allowNull) {
      b.if(this._checkNull(prop), Builder.block([
  ...
  _addOptionalStringCheck(prop) {
    ...
    block.if(this._getTypeof(prop), Builder.block(
      this._callbackWithError(this._getBuiltinTypeError(prop))
    ))

    if (typeof prop.min === 'number') {
      block.if(this._getStringLessThan(prop), Builder.block(
        this._callbackWithError(this._getStringMinError(prop))
      ))
    }

    if (typeof prop.max === 'number') {
      block.if(this._getStringGreaterThan(prop), Builder.block(
        this._callbackWithError(this._getStringMaxError(prop))
      ))
    }

    if (prop.allowNull) {
      b.if(
        Builder.and(this._getHas(prop), this._checkNull(prop))
      , Builder.block(block.build())
      )
    } else {
  ...
  _callbackWithError(err) {
    return Builder()
      ...
      .returns(Builder.callFunction('setImmediate', [
        E.ARROW([], Builder.block([
          S.EXPRESSION(Builder.callFunction('cb', [Builder.id('er')]))
        ]))
      ]))
      .build()
  }
