...
module.exports = class Validator {
  ...
  build() {
    ...
    this.addCheck(this.root, this.rootPath, body)

    body.push(...utils.callbackSuccess([
      Builder.id('null')
    , Builder.id(this.rootPath)
    ]))

    return body.build()
  }

  addCheck(prop, path, builder) {
    const CheckClass = ({
      'array': ArrayType
    , 'boolean': BaseType
    , 'date': BaseType
    ...
    const check = new CheckClass(prop, path, builder, this)
    check.generate()
  }

  _getHasOwnProperty(path) {
    return utils.getHasForPath(path)
  }
  ...
  _getBuiltinTypeError(check) {
    ...
    const displayPath = utils.removeRootFromPath(check.displayPath)
