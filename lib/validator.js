...
Validator.prototype._processModule = function _processModule() {
  ...
  if (types.email) {
    this.builder1
      .declare('const', 'tld', Builder.require('tldjs'))
      .declare('const', 'emailRE', Builder.regex(
...
Validator.prototype.generate = function generate() {
  // ast.body[0] is isDate()
  // ast.body[1] is isv4UUID()
  // ast.body[2] is isUUID()
  // ast.body[3] is isEmail()

  const isUUID = this.ast.body.pop()
  const isUUIDv4 = this.ast.body.pop()
  const isDate = this.ast.body.pop()
  const isEmail = this.ast.body.pop()
  this._processModule()
  this._processBody()
  ...
  if (this.types.email) {
    this.builder.body.push(isEmail)
  }

  if (this.types.date) {
    this.builder.body.push(isDate)
  }

  if (this.types.uuid) {
    this.builder.body.push(isUUIDv4)
    this.builder.body.push(isUUID)
  }

  return fixup(escodegen.generate(this.builder.program(), genOpts))
}
