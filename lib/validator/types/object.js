...
module.exports = class ObjectCheck extends Base {
  ...
  _generateInitialCopy() {
    if (this.prop.passthrough) return Builder.objectPath(this.path)
  ...
  _generateRequired() {
    ...
    if (this.validator.stripExtraneousProperties
      && prop.props === null
      && !prop.passthrough) {
      throw new Error(`${prop.path} requires either .props or .passthrough`)
    }
    ...
    if (this._isTrivial() && !prop.passthrough) {
    ...
    if (this.validator.failOnExtraneousProperties && !this._isTrivial()) {
